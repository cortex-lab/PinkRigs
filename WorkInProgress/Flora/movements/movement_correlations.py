...
m = movement_correlation(dataset=which,recompute_data_selection=True)

# %%
corrv,iscorr = m.get_corrs(tbin=.05)
...
# Plotting
fraction_good.plot(kind='bar', color='skyblue',ax=ax)
...
plt.hist(gSC.corrVal[gSC.is_corr],bins=100,alpha=.5)
plt.hist(gSC.corrVal[~gSC.is_corr],bins=100,alpha=.5)
...
ax[0].set_ylim([-3100,-700])
ax[0].set_title(' movement cells in SC')

anat.plot_anat_canvas(ax=ax[1],axis = 'dv',coord = 2000)
anat.plot_points(gSC.gauss_ml.values, gSC.gauss_ap.values,s=20,color='grey',alpha=0.3,unilateral=True)
anat.plot_points(sig.gauss_ml.values, sig.gauss_ap.values,s=40,color=dot_colors,alpha=1,edgecolors='k',unilateral=True)

ax[1].set_xlim([-2200,0])
ax[1].set_ylim([-4500,-2500])
