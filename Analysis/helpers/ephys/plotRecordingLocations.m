...
for ee = 1:size(exp2checkList,1)
    ...
    for pp = 1:numel(ephysPaths)
        ...
        if isempty(binFile)
            fprintf('No bin file in %s\n. Skipping',ephysPaths{pp})
        else
            ...
            while any(all(textLoctmp == textLoc,2)) % Dirty way to avoid overlap
                textLoctmp = textLoctmp-[0 250];
            end
            textLoc = [textLoc; textLoctmp];
            text(textLoctmp(1),textLoctmp(2),[expInfo.expDate{1} '_' expInfo.expNum{1}],'color',protocolColor)
            
            rectangle('Position',[miXpos-ee*5, miDepth, maXpos-miXpos+ee*10, maDepth-miDepth], ...
                'EdgeColor',protocolColor,'LineWidth',3)
        end
    end
end   