function [tVid,numFramesMissed,nFirstFrames] = video(varargin)
    %% Aligns the time frames of the input video to the corresponding timeline.
    % Note: uses the IR light "dark" flashes at the beginning and end of an
    % experiment. It will try to minimize the amount of time and computing 
    % by loading first only the beginning and end of the file. It will then
    % iterate until it finds both flashes.
    ...
    % recomputeInt: bool
    %   Whether to recomputes the movie intensities.
    % nFramesToLoad: int
    %   Number of frames to use at first.
    % adjustPercExpo: bool
    %   Will adjust the timing based on the percentage of exposition of the
    %   last frame in the first dark flash.
    % plt: bool
    %   Whether to plot the interframe interval.
    % crashMissedFrames: bool
    %   Whether to crash the code if there are missed frames.
    %
    % Returns: 
    % -------------------
    % tVid: vector
    %   Video times in timeline time
    % numFramesMissed: int
    %   Number of missed frames
    % nFirstFrames: int
    %   Number of frames in the main movie.
