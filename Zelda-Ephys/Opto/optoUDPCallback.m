function optoUDPCallback(src, event)
...
data=fread(src);
str=char(data');
tag = ['[optoUDP ', datestr(t, 'HH:MM:SS.FFF'), '] '];
...
switch info.instruction
    ...
    case 'ExpStart'
        ...
        file2save = fullfile(filePath, [fileStem, '_opto.mat']);
        [filePath, fileStem] = dat.expPath(info.expRef, 'main', 'master');
        fileOnServer = fullfile(filePath, [fileStem, '_opto.mat']);
        adi = audiodevinfo;
...
%             {adi.input.Name});
    ...
    case {'ExpEnd', 'ExpInterrupt'}
        pause(30); % time to get the end buzzer
        stop(ar);
        Fs = ar.SampleRate;
